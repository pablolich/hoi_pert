...
function parametersubstitution(par_syst, par_value, pars)
    ...
    for i in 1:neqs
 	par_syst.expressions[i] = subs(par_syst.expressions[i] , pars => par_value)
    end
    return par_syst
end
...
endpars:
"""
function traversealpha(par_syst, initialsol, startpars, endpars, sim, n)
    ...
    res = solve(par_syst, compile = false)
	...
	return traversealpha(par_syst, initialsol, startpars, endpars, sim, n)
    end
end
...
function main()
	...
	@var x[1:n]
	    ...
	    traversealpha(syst, ones(n), [.0], [.0+initialstep], sim, n)
	end
    end
    return 0
end
